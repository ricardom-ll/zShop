...
if  (isset($_GET['idslide'])){
    $idslide=filter_input(INPUT_GET,'idslide',FILTER_VALIDATE_INT);
    if  ($idslide==false){
    ...
    if  (isset($_POST['aceptar'])){
        ...
        if (!isset($errores)){
          ...
          try {
            $sql="UPDATE slides set nombre=:nom where id=:x";
            $stmt=$con->prepare($sql);
            if  ($stmt->execute([":nom"=>$nombre,":x"=>$idslide])){
            }
          } catch (PDOException $e){
        ...
        if (isset($_POST['aceptar']) && is_uploaded_file($_FILES['imagen']['tmp_name'])){
            ...
            if ($_FILES['imagen']['type']=="image/jpeg" || $_FILES['imagen']['type']=="image/jpg"){
                if(!isset($errores)){
                  ...
                  //Realizamos el UPDATE de nuestra BDD con la nueva informacion
                  try {
                      $sql="UPDATE slides set imagen=:img where id=:x";
                      $stmt=$con->prepare($sql);
                      if  ($stmt->execute([":img"=>$nombreDefinitivo,":x"=>$idslide])){
                          if  ($stmt->rowCount()!=1){
  ...
  <section>
    ...
    ?>
            <form method="post" enctype="multipart/form-data">
                ...
                <input type="hidden" name="idcat" value="<?=$idslide?>"/>
